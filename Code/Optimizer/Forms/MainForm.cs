...
namespace Optimizer {
    public sealed partial class MainForm : Form {
        ...
        string _feedImages = "https://raw.githubusercontent.com/hellzerg/optimizer/master/images/feed.zip";
        ...
        private void GetFeed() {
            ...
            try {
                ...
                _feedImages = "https://drive.usercontent.google.com/u/0/uc?id=1BwEj4P8P-weJu5TSwJkuAJDIC7QmaDtA&export=download";
                ...
                using (ZipArchive zip = new ZipArchive(new MemoryStream(feedData))) {
                    ...
                    try {
                        ...
                        if (File.Exists("feed.json"))
						{
                            feed = File.ReadAllText("feed.json");
                        }
                        AppsFromFeed = JsonConvert.DeserializeObject<List<AppInfo>>(feed);
                        // Testing by changing in feed.json file
                        AppInfo foundObject = AppsFromFeed.FirstOrDefault(obj => obj.Tag == "c7zip");
                        AppCard appCard;
        ...
        private async void UninstallModernApps() {
            ...
            if (MessageBox.Show(_removeModernAppsMessage, "RGBTool", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes) {
                uninstallModernAppsButton.Enabled = false;
                refreshModernAppsButton.Enabled = false;
                ...
                if (!string.IsNullOrEmpty(failedApps)) {
                    MessageBox.Show(_errorModernAppsMessage + failedApps, "RGBTool", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
        ...
        private void button32_Click(object sender, EventArgs e) {
            ...
            if (MessageBox.Show(_removeStartupItemsMessage + Environment.NewLine + Environment.NewLine + report, "RGBTool", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes) {
        ...
        private void btnResetConfig_Click(object sender, EventArgs e) {
            if (MessageBox.Show(_repairMessage, "RGBTool", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes) {
                Utilities.Repair();
            }
        }
        ...
        private void toggleSwitch4_Click(object sender, EventArgs e) {
            if (systemRestoreSw.ToggleChecked) {
                if (MessageBox.Show(OptionsHelper.TranslationList["systemRestoreM"].ToString(), "RGBTool", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No) {
        ...
        private void toggleSwitch14_Click(object sender, EventArgs e) {
            if (uODSw.ToggleChecked) {
                if (MessageBox.Show(OptionsHelper.TranslationList["onedriveM"].ToString(), "RGBTool", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No) {
                    _skipOneDrive = true;
        ...
        private void RenderAppDownloaderBusy() {
            ...
            Program._SplashForm = new SplashForm();
            var splashThread = new Thread(new ThreadStart(
                () => Application.Run(Program._SplashForm)));
        ...
        private void RenderAppDownloaderFree() {
            ...
            if(Program._SplashForm != null && !Program._SplashForm.Disposing && !Program._SplashForm.IsDisposed)
                Program._SplashForm.Invoke(new Action(() => Program._SplashForm.Close()));
        }
        ...
        private async void btnDownloadApps_Click(object sender, EventArgs e) {
            if (string.IsNullOrWhiteSpace(txtDownloadFolder.Text) || !Directory.Exists(txtDownloadFolder.Text)) {
                MessageBox.Show(OptionsHelper.TranslationList["downloadDirInvalid"].ToString(), "RGBTool", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }

            RenderAppDownloaderBusy();
            List<string> pathInstalled = new List<string>();
            maxCount = 0;
            count = 0;
            ...
            Control[] temp;
            List<string> appChecked = new List<string>();
            foreach (AppInfo x in AppsFromFeed) {
            ...
            if (cAutoInstall.Checked) {
                ...
                string[] f = Directory.GetFiles(txtDownloadFolder.Text, "*.*", SearchOption.TopDirectoryOnly);
                string[] filteredFilePaths = f
                        .Where(path => (path.EndsWith(".exe", StringComparison.OrdinalIgnoreCase) ||
                        path.EndsWith(".msi", StringComparison.OrdinalIgnoreCase) || path.EndsWith(".msixbundle", StringComparison.OrdinalIgnoreCase)))  .ToArray();

                filteredFilePaths = filteredFilePaths.Where(a => appChecked.Any(b => a.Contains(b))).ToArray();
                foreach (string a in filteredFilePaths) {
                    pathInstalled.Add(a);
                    using (p = new Process()) {
                        count++;
                        ...
                        // APP-SPECIFIC HACKS //
                        if (a.ToLowerInvariant().Contains("sumatra")){
            ...
            /* Install all of .bat file in the (Zip File) */
            List<string> speciallyApp = new List<string>() { "Visual C++ AIO", "VS Studio", "Rounded Taskbar", "StarWard", "DDU" };

            byte[] zipData;

            string[] fwinRar = Directory.GetFiles(txtDownloadFolder.Text, "*.*", SearchOption.TopDirectoryOnly);
            string[] filezip = fwinRar.Where(path => (path.EndsWith(".zip", StringComparison.OrdinalIgnoreCase) ||
                                    path.EndsWith(".rar", StringComparison.OrdinalIgnoreCase) || path.EndsWith(".7z", StringComparison.OrdinalIgnoreCase))).ToArray();

            filezip = filezip.Where(a => speciallyApp.Any(b => a.Contains(b))).ToArray(); /* filezip must be contain one of path sepeciallyApp */
            filezip = filezip.Except(pathInstalled).ToArray();/* filezip must be not contain app already installed */
            
            foreach (var zFiles in filezip)
			{
                if (!cAutoInstall.Checked) break;
                if (Path.GetExtension(zFiles)==".7z")
				{
                    using (var extractor = new SevenZipExtractor.ArchiveFile(zFiles))
                    {
                        string destinationFilePath = Path.Combine(txtDownloadFolder.Text);
                        extractor.Extract(destinationFilePath, true);
                        string pathFolder = Directory.GetParent((extractor.Entries[0].FileName)).Name;

                        string pathRun = Path.Combine(txtDownloadFolder.Text, pathFolder);
                        string[] exeFiles = Directory.GetFiles(pathRun, "*.*", SearchOption.TopDirectoryOnly);
                        exeFiles = exeFiles.Where(path => path.EndsWith(".exe", StringComparison.OrdinalIgnoreCase)).ToArray();

						foreach (var item in exeFiles)
						{
                            if (pathInstalled.Contains(item)) continue;
                            using (p = new Process())
                            {
                                count++;
                                p.StartInfo.FileName = item;
                                p.EnableRaisingEvents = true;
                                p.StartInfo.WorkingDirectory = pathRun;
                                await p.RunAsync();
                                p.Close();
                            };
                        }
				...
				{
                    zipData = File.ReadAllBytes(zFiles);
                    try
                    {
                        using (ZipArchive zip = new ZipArchive(new MemoryStream(zipData)))
                        {
                            var zipEntries = zip.Entries;
                            try
                            {
                                string newFolder;
                                var filteredEntries = zipEntries.Where(entry => (Path.GetExtension(entry.FullName) == ".bat")).ToList();
								if (filteredEntries.Count() >0)
								{
                                    /*Transmission all file after decompress if have .bat file*/
                                    foreach (var entry in zipEntries)
                                    {
                                        newFolder = Path.GetFileNameWithoutExtension(zFiles);
                                        if (!Directory.Exists(Path.Combine(txtDownloadFolder.Text, newFolder))){
                                            Directory.CreateDirectory(Path.Combine(txtDownloadFolder.Text, newFolder));
										}
                                        string destinationFilePath = Path.Combine(txtDownloadFolder.Text, newFolder, entry.Name);
								...
								{
                                    /* Before run exe file in the zip file need copy to File */
                                    filteredEntries = zipEntries.Where(entry => (Path.GetExtension(entry.FullName) == ".exe")).ToList();
                                    foreach (var entry in filteredEntries)
                                    {
                                        ...
                                        string destinationFilePath = Path.Combine(txtDownloadFolder.Text, newFolder, entry.FullName);
                                        using (Stream entryStream = entry.Open())
                                        {
                                            using (FileStream destinationStream = File.Create(destinationFilePath))
                                            {
                                ...
                                {
                                    if (pathInstalled.Contains(item.FullName)) continue;
                                    using (p = new Process())
                                    {
                                        count++;
                                        p.StartInfo.FileName = Path.Combine(txtDownloadFolder.Text, newFolder, item.FullName);
                                        p.EnableRaisingEvents = true;
                                        p.StartInfo.WorkingDirectory = txtDownloadFolder.Text;
                                        await p.RunAsync();
                                        p.Close();
                                ...
                                /*Delete all of file*/ /* Currently do nothing */
                            }
                            catch (Exception ex)
                            {
                                Logger.LogError("MainForm.GetFeed", ex.Message, ex.StackTrace);
        ...
        private string[] allowableZip = new string[] { "cDdu", "cVisualc++aio", "cRoundedtaskbar",  "cStarward" };
        private async Task DownloadApp(AppInfo app, bool pref64) {
            try {
                using (WebClient downloader = new WebClient()) {
                    ...
                    if (pref64) {
                        if (app.Tag == "cTranslucenttaskbar"){
                            fileExtension = ".msixbundle";
                        }
                        else if (app.Tag == "cStarward") {
                            fileExtension = ".7z";
                        }
                        else if (allowableZip.Contains(app.Tag)){
                            fileExtension = ".zip";
                        }
                        else if(app.Link64.Contains(".msi")) {
                            fileExtension = ".msi";
                        }
                        else if (app.Link64.Contains(".exe")) {
                            fileExtension = ".exe";
                        }
						else
						{
        ...
        private void btnViewLog_Click(object sender, EventArgs e) {
            if (File.Exists(Logger.ErrorLogFile)) {
                ...
                MessageBox.Show(OptionsHelper.TranslationList["noErrorsM"].ToString(), "RGBTool", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        ...
        private void btnExport_Click(object sender, EventArgs e) {
            if (ExportDialog.ShowDialog() == DialogResult.OK) {
                try {
                    ...
                    MessageBox.Show(ex.Message, "RGBTool", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
        }
        ...
        private void flushCacheB_Click(object sender, EventArgs e) {
            if (MessageBox.Show(_flushDNSMessage, "RGBTool", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes) {
        ...
        private string ParseChangelog() {
            ...
            if (changelogText.Count <= 0) {
                MessageBox.Show(_noNewVersionMessage, "RGBTool", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return string.Empty;
            }
        ...
        private void btnRestoreUwp_Click(object sender, EventArgs e) {
            if (MessageBox.Show(_uwpRestoreMessage, "RGBTool", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes) {
        ...
        private void btnReinforce_Click(object sender, EventArgs e) {
            if (MessageBox.Show(_reinforcePoliciesMessage, "RGBTool", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes) {
        ...
        private void autoUpdateToggle_ToggleClicked(object sender, EventArgs e) {
            if (!OptionsHelper.CurrentOptions.UpdateOnLaunch && autoUpdateToggle.ToggleChecked == true)
			{
                CheckForUpdate();
            }
		...
		private void setDownDirLbl_Click(object sender, EventArgs e)
		{
		...
		private void lblVideoSound_Click(object sender, EventArgs e)
		{

		}

		private void progressDownloader_Click(object sender, EventArgs e)
		{

		}

		private void lblBackupTitle_Click(object sender, EventArgs e)
		{

		}

        private readonly string advanced_tweak_bat = "advanced_tweak.bat";
        private readonly string revert_tweak_bat = "revert_tweak.bat";
        private async void  tweaking_clicked(object sender, MouseEventArgs e)
		{
            if (!File.Exists(Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), advanced_tweak_bat))) return;
            string mess = OptionsHelper.TranslationList["notificationAvanceTweaking"];
            MessageBox.Show(mess, "Advance Tweaking", MessageBoxButtons.OK, MessageBoxIcon.Information);

            /* Optimizing  Busy */
            Program._MainForm.Enabled = false;
            ...
            using (p = new Process())
            {
				try
				{
                    p.StartInfo.FileName = advanced_tweak_bat;
                    p.EnableRaisingEvents = true;
            ...
            Program._MainForm.Enabled = true;
        }

        private async void revert_clicked(object sender, EventArgs e)
		{
            if (!File.Exists(Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), revert_tweak_bat))) return;
            /* Reverting busy*/
            Program._SplashForm = new SplashForm();
            var splashThread = new Thread(new ThreadStart(
                () => Application.Run(Program._SplashForm)));
            ...
            using (p = new Process())
            {
				try
				{
                    p.StartInfo.FileName = revert_tweak_bat;
                    p.EnableRaisingEvents = true;
                    p.StartInfo.WorkingDirectory = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
                    await p.RunAsync();
                    p.Close();
