...
namespace Optimizer {
    static class Program {
        ...
        static MainForm getMainForm() { return _MainForm; }

        internal static SplashForm _SplashForm;
        static SplashForm getSplashForm() { return _SplashForm; }

        static string _adminMissingMessage = "RGBTool needs to be run as administrator!\nApp will now close...";
        static string _unsupportedMessage = "RGBTool works with Windows 7 and higher!\nApp will now close...";
        ...
        static string _alreadyRunningMsg = "RGBTool is already running in the background!";
        ...
        static void Main(string[] switches) {
            ...
            if (!_notRunning) {
                MessageBox.Show(_alreadyRunningMsg, "RGBTool", MessageBoxButtons.OK, MessageBoxIcon.Information);
                Environment.Exit(0);
                return;
            }
            ...
            if (switches.Length == 1) {
                ...
                if (arg == "/version") {
                    if (!EXPERIMENTAL_BUILD) MessageBox.Show($"RGBTool: {GetCurrentVersionTostring()}\n\nCoded by: RGBTool © ∞\n\nhttps://github.com/paimon3107/codeapp", "RGBTool", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    else MessageBox.Show("RGBTool: EXPERIMENTAL BUILD. PLEASE DELETE AFTER TESTING.\n\nCoded by: RGBTool © ∞\n\nhttps://github.com/paimon3107/codeapp", "RGBTool", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                if (arg == "/disabledefender") {
                    ...
                    MessageBox.Show("Windows Defender has been completely disabled successfully.", "RGBTool", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    Environment.Exit(0);
                    return;
                }
                ...
                if (arg.StartsWith("/config=")) {
                    ...
                    if (!File.Exists(fileName)) {
                        MessageBox.Show(_confNotFoundMsg, "RGBTool", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        Environment.Exit(0);
                        return;
                    }
                    ...
                    if (SilentOps.CurrentSilentConfig == null) {
                        MessageBox.Show(_confInvalidFormatMsg, "RGBTool", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        Environment.Exit(0);
                        return;
                    }
                    if (!SilentOps.ProcessWindowsVersionCompatibility()) {
                        MessageBox.Show(_confInvalidVersionMsg, "RGBTool", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        Environment.Exit(0);
                        return;
                    }
                    SILENT_MODE = true;
        ...
        private static void DisableDefenderInSafeMode() {
            if (File.Exists("DisableDefenderSafeMode.bat"))
			{
        ...
        private static void EnableDefenderInSafeMode() {
            if (File.Exists("EnableDefenderSafeMode.bat"))
			{
